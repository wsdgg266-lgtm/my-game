...
- **📷 値札の写真読み取り**: お得計算で「値札を撮影」すると、写真から価格と内容量(g/ml/L)を自動で読み取って入力。複数の縮尺で3〜4回解析して統合する方式で、大きな価格数字と小さな容量表記の両方を拾う。候補が複数あるときはタップで選び直せる。OCR(Tesseract)はアプリに同梱で、写真は端末の外に送信されない
...
連写と比較明合成で、スマホでも花火の光跡がきれいに伸びた1枚を撮れるカメラアプリ
...
B.LEAGUE・NBA・バスケットボール日本代表の試合日程、結果、順位表、選手成績、
そして日本のバスケットボール関連ニュースがまとめて見られるアプリ
...
あらかじめ用意された39個の枠に写真をはめていく作りで、空き枠をタップするとカメラが開き、
四隅の検出・台形補正・背景消し・枠へのはめこみまで操作なしで進みます。
一度登録したシールをまた撮ると写真で見分けて知らせ、1タップでダブりにできます。
キラ(箔)かどうかも写真から推定してレアリティを先に選びます。
ダブりの枚数・交換リスト・レアリティ別のコンプ率も出せます。
